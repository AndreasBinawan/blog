...
<div id="main">
    ...
    @foreach ($data as $row_post)
        ...
        <article class="post">
            ...
            <p>{{ Str::limit( $row_post->news, 500) }}</p>
            <footer>
                <ul class="actions">
                    <li><a href="{{ url('detail/'.str::slug($row_post->title, '-')) }}" class="button big">Continue Reading</a></li>
                </ul>
            </footer>
        </article>
        @endforeach

    {{$data->links('frontend.custom.pagination')}}
            ...
            <div class="mini-posts">
                ...
                <!-- Mini Post -->
                    ...
                    <article class="mini-post">
                        <header>
                            <h3><a href="{{ url('detail/'.str::slug($row_mini_post->title, '-')) }}">{{ $row_mini_post->title }}</a></h3>
                            <time class="published" datetime="2015-10-20">{{ \Carbon\Carbon::parse($row_mini_post->created_at)->format('d F Y') }}</time>
                            <a href="#" class="author"><img src="{{ asset('assets/frontend/images/'.$row_mini_post->icon) }}" alt="" /></a>
                        </header>
                        <a href="#" class="image"><img src="{{ asset('assets/frontend/images/'.$row_mini_post->images) }}" alt="{{ $row_mini_post->alt }}" /></a>
                    </article>
                @endforeach
